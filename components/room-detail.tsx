  ...
  return <main className="mx-auto min-h-screen max-w-xl p-5 pb-12">
    ...
    <section className="mt-5">
      ...
      {tab === "people" ? <div className="mt-3 rounded-3xl border border-stone-200 bg-white px-4">{room.orders.map((order) => {
        ...
        return <div key={order.teamMemberId} className="border-b border-stone-100 py-4 last:border-0"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-full bg-stone-100 font-black text-stone-600">{user.name.slice(0, 1)}</span><div className="min-w-0 flex-1"><p className="font-bold">{user.name}</p><p className="mt-0.5 text-xs text-stone-500">{selectedMenu ? `${selectedMenu.name} ${order.temperature}` : order.status === "ABSENT" && order.markedByMemberId ? `${memberName(order.markedByMemberId)}님이 표시` : ""}</p>{delegated && <p className="mt-0.5 text-[11px] text-stone-400">{memberName(order.selectedByMemberId)}님이 대신 선택</p>}</div><OrderStatusBadge status={order.status} /></div>
