...
export function ShareOrderActions({ title = "커피 주문", path, compact = false }: { title?: string; path?: string; compact?: boolean }) {
  ...
  if (compact) return <div className="flex gap-1.5"><button type="button" onClick={() => void copy()} aria-label="주소 복사" title="주소 복사" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-stone-100 px-3 text-xs font-bold text-stone-700 transition hover:bg-stone-200 active:scale-95">{copied ? <Check size={16} /> : <LinkIcon size={16} />}<span className="hidden sm:inline">{copied ? "복사됨" : "주소 복사"}</span></button><button type="button" onClick={() => void share()} aria-label="공유하기" title="공유하기" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-stone-100 px-3 text-xs font-bold text-stone-700 transition hover:bg-stone-200 active:scale-95"><Share2 size={16} /><span className="hidden sm:inline">공유하기</span></button></div>;
