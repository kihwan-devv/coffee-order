...
export const cafe = (row: Row): Cafe => ({ id: row.id, name: row.name, emoji: row.emoji ?? "☕", color: row.color ?? "bg-amber-100", logoUrl: row.logo_url ?? null, imageUrl: row.image_url ?? null, officialMenuUrl: row.official_menu_url ?? null, isActive: row.is_active ?? true, sortOrder: row.sort_order ?? 0 });
export const menu = (row: Row): Menu => ({ id: row.id, cafeId: row.cafe_id, name: row.name, category: row.category ?? null, imageUrl: row.image_url ?? null, supportedTemperatures: row.available_temperatures ?? row.supported_temperatures ?? [], isActive: row.is_active ?? true, sortOrder: row.sort_order ?? 0 });
