...
export const order = (row: Row): OrderRoom => ({ id: row.id, teamId: row.team_id, orderCode: row.order_code, name: row.title, cafeId: row.cafe_id, createdBy: row.created_by_member_id, deadline: row.deadline, status: row.status, createdAt: row.created_at, orders: (row.order_responses ?? []).map(response) });
