...
import { order, teamMember, type Row } from "./shared";
import type { OrderRoom, OrderStatus, TeamMember, Temperature } from "@/types";
...
export async function getOrderDetail(teamId: string, orderCode: string): Promise<{ order: OrderRoom; members: TeamMember[] }> {
  ...
  return {
    ...
    members: membersResult.data.map(teamMember),
  };
}
...
export async function deleteOrder(orderId: string) { const { data, error } = await createClient().from("orders").delete().eq("id", orderId).select("id").maybeSingle(); if (error) { console.error("delete order error", { code: error.code, message: error.message, details: error.details, hint: error.hint }); throw error; } if (!data?.id) throw new Error("주문을 삭제할 권한이 없거나 주문을 찾을 수 없습니다."); }
